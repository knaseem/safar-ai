...
export default function AdminDashboardClient({ initialBookings, initialProfiles }: AdminDashboardClientProps) {
    ...
    useEffect(() => {
        // Pre-process analytics using server-fetched data
        processAnalytics(initialBookings, initialProfiles)
        // Only fetch users on the client to keep the initial server load lighter
        fetchUsers()
    }, [])

    // Removed fetchAllData wrapper since we fetch users directly now
    ...
    const refetchBookings = async () => {
        try {
            ...
            if (data.bookings) {
                ...
                // We keep profiles as initial since they rarely change during a session
                processAnalytics(data.bookings, initialProfiles)
            }
        } catch (err) {
